# Perf Agent — 매뉴얼 (멀티엔진: SQL Server / PostgreSQL / MySQL)

DBAOps와 **A2A 프로토콜로 상호 연동**되는 **멀티엔진 쿼리 성능 에이전트** (LangGraph).
docker 없이 **venv + systemd**로 DBAOps 4개 + dbaops-a2a + perf 3개(a2a/streamlit/slack-bot) 서비스가 한 호스트에서 돈다.
...
- **dbperf-a2a (:9100)** — SQL Server 쿼리 성능 전문가. **LangGraph 4노드 파이프라인**
  ...
  stdio MCP 도구 13개(mssql/postgres/mysql 자동 분기) + ask_dbaops_agent(A2A, 양방향).
