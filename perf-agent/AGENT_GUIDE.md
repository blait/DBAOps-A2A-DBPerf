...
# CLI (컨테이너 안)
docker compose exec perf-a2a python query_agent.py            # 대화형
docker compose exec perf-a2a python query_agent.py "지난 24시간 CPU 상위 쿼리 5개 보여줘"
```

분석 대상 DB를 바꾸려면 `.env`의 `DB_NAME` 수정 후 `docker compose up -d perf-a2a`.
...
# 2. 에이전트로 진단
docker compose exec -e DB_NAME=DBOpsLab perf-a2a python query_agent.py "CPU가 높은 원인 쿼리를 찾고 인덱스 개선안을 만들어줘"

# 3. 에이전트가 추천한 CREATE INDEX 적용 (또는 04_create_indexes_fix.sql 실행)

# 4. 개선 확인
docker compose exec -e DB_NAME=DBOpsLab perf-a2a python query_agent.py "인덱스 적용 후 성능이 개선됐는지 회귀 쿼리 기준으로 비교해줘"
```
