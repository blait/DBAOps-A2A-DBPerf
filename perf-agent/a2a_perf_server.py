"""
a2a_perf_server.py - Query Performance 에이전트를 A2A 프로토콜로 노출 (:9000).

DBAOps ops agent(파사드 :9001)나 다른 A2A 클라이언트가 이 서버로
SQL Server 쿼리 성능 질문을 보낼 수 있다.

Agent card: http://<host>:9000/.well-known/agent-card.json
...
PORT = int(os.environ.get('A2A_PERF_PORT', '9000'))
...
def main():
    ...
    with mcp_client:
        # 양방향 A2A: 이 서버의 에이전트도 ops 파사드(:9001)에 물어볼 수 있다.
